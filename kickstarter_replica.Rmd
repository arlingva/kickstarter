...
df.CDF <- df %>%
  ...
  arrange(usd_goal_real) %>%
  mutate(acumulado =  (usd_goal_real),
...
df.CDF <- df %>%
  select(goal.alpha) %>%
  arrange(goal.alpha) %>%
  filter(!is.na(goal.alpha)) %>%
  mutate(acumulado = cumsum(goal.alpha),
         CDF = acumulado / sum(goal.alpha))

fig <- plot_ly(x = ~df.CDF$goal.alpha, 
               y = df.CDF$CDF, type = 'scatter', mode = 'lines')
fig
```
...
fig <- plot_ly(df.CDF, x = ~goal.alpha, 
