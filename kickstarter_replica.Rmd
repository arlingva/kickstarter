...
```{r}
100 * length(df0[which(df0$backers > 100), 1]) / length(df0[,1])
```
...
Solo el 35.4 % de las campañas logran completarse con éxito.
...
df <- df %>%
  ...
  mutate(rangosDuracion = case_when(
    ...
    TRUE ~ "Más de 92 días"))
#vec.categorias <- unique(df$rangosDuracion)
```
...
```{r}
fig <- plot_ly(df, y = ~duracion, type = "box")
...
fig <- plot_ly(df.tabla, x = ~columna, y = ~Freq, 
               ...
               ylab = "Tasa de éxito", xlab = "Duración de campaña")
...
```{r}
summary(df$duracion)
paste("Moda: ", mfv(df$duracion))
paste("Desviación estándar: ", sd(df$duracion))
```
...
### Duración en rango por estado de campaña
...
Resumen estadístico de usd_goal_real
...
#### Distribución de usd_goal_real

Descartando outliers, se elimina al \alpha \% de los datos extremos

```{r}
alpha <- 0.05

goal1 <- descarta.outliers(df$usd_goal_real, alpha)
hist(goal1, main = "usd_goal_real sin outliers", freq = TRUE,
